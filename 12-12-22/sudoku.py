...
def crear() -> Sudoku:
  graella: list[list[Optional[None]]] = [[None for _ in range(9)] for _ in range(9)]
...
def llegir() -> Sudoku:
  ...
  for i in range(9):
    for j in range(9):
      v = read(str)
      if s != ".":
        marcar_casella(s, i, j, int(s))
  return s

def marcar_casella(s: Sudoku, i: int, j: int, v: int) -> None:
  s.graella[i][j] = v
  s.files[i][v] = True
  s.columnes[j][v] = True
  s.caixes[caixa(i, j)][v] = True

def desmarcar_casella(s: Sudoku, i: int, j: int, v: int) -> None:
  s.graella[i][j] = None
  s.files[i][v] = False 
